...
def convolve_grayscale(images, kernel, padding='same', stride=(1, 1)):
    ...
    Args:
        ...
        padding (str or tuple): 'same', 'valid', or (ph, pw)
            - 'same': Zero padding added to ensure the output has the same dimensions as the input.
            - 'valid': No padding, output will shrink based on kernel size.
            - (ph, pw): Custom padding for the height (ph) and width (pw) of the image.
    ...
    images_padded = np.pad(images, pad_width=npad, mode='constant', constant_values=0)
    ...
    for i in range(nh):
        ...
        for j in range(nw):
            ...
            # Perform element-wise multiplication between image slice and kernel, and sum the result
